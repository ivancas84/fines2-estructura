...
<? foreach($comision_cursos_horarios as $comision => $cursos):
  ...
  <tr>
    ...
    <td rowspan="<?=count($cursos)?>">PROVISIONAL</td>
    <td rowspan="<?=count($cursos)?>">FINES TRAYECTO SECUNDARIO</td>
    <td rowspan="<?=count($cursos)?>">100</td>
    <td rowspan="<?=count($cursos)?>">ADULTOS</td>
    <td rowspan="<?=count($cursos)?>"><?=$d["centro_educativo"]->_get("nombre")?></td>
    ...
    <td rowspan="<?=count($cursos)?>"><?=$d["plan"]->_get("orientacion","X")?></td>
    <td rowspan="<?=count($cursos)?>"><?=$d["planificacion"]->_get("anio")."°".$d["planificacion"]->_get("semestre")."C"?></td>
    <td rowspan="<?=count($cursos)?>"><?=$d["comision"]->_get("turno","X")?></td>
    <td rowspan="<?=count($cursos)?>">SIMPLE</td>
    <td rowspan="<?=count($cursos)?>">30/08/2021</td>
    <td rowspan="<?=count($cursos)?>">31/12/2021</td>
