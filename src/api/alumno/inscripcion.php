...
class AlumnoInscripcionApi extends BaseApi {
  ...
  public function persona(){
    $this->persona = $this->container->value("persona")->_fromArray($this->data["persona"], "set");
  }
  ...
  public function persistPersonaId(){
    $persist = $this->container->controller("persist_sql_value", "persona")->id($this->persona);
    $this->sql .= $persist["sql"];
    array_push($this->detail, "persona".$this->persona->_get("id"));
  }

  public function persistPersonaUnique(){
    $persist = $this->container->controller("persist_sql_value", "persona")->unique($this->persona);
    $this->sql .= $persist["sql"];
    array_push($this->detail, "persona".$this->persona->_get("id"));
  }
  ...
  public function persistAlumnoUnique(){
    ...
    $persist = $this->container->controller("persist_sql", "alumno")->unique($row);
    $this->sql .= $persist["sql"];
    array_push($this->detail, "alumno".$row["id"]);
  }

  public function insertDomicilio(){
    $p = $this->container->controller("persist_sql", "domicilio")->id($this->data["dom"]);
    $this->sql .= $p["sql"];
  ...
  public function insertDetallePersona_(){
    foreach($this->data["detalle_persona/persona"] as $dp){
      ...
      $persist = $this->container->controller("persist_sql", "detalle_persona")->id($dp);
      $this->sql .= $persist["sql"];
