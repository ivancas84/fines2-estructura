...
class TransferirAlumnoComisionApi extends BaseApi {
  ...
  protected function persistirAlumnos(){
    foreach($this->alumnoComisionArray as $idPersona => $alumnoComision){
      $ac = $this->container->tools("alumno_comision")->value($alumnoComision)["alumno_comision"];
      ...
      if(array_key_exists($idPersona, $this->alumnoArray)){
        $a = $this->container->tools("alumno")->value($this->alumnoArray[$idPersona])["alumno"];
      ...
      $persist = $this->container->controller("persist_sql_value", "alumno")->id($a);
