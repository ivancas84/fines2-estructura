...
class TransferirAlumnoApi extends BaseApi {
  ...
  public function actualizarPersona(){
    ...
    $persist = $this->container->controller("persist_sql", "persona")->id($p);
  ...
  public function transferirEntidadAlumnoComision(){
    ...
    foreach($transferir as $comisionAt => $alumnoComision){
      $at = $this->container->tools("alumno_comision")->value($alumnoComision)["alumno_comision"];

      if(in_array($comisionAt, $existentes)){
        $ae = $this->container->tools("alumno_comision")->value($existentes[$comisionAt])["alumno_comision"];
  ...
  public function transferirEntidadAlumno(){
    ...
    $at = $this->container->value("alumno")->_fromArray($this->alumnoTransferir,"set");

    if(!empty($this->alumnoExistente)){
      $ae = $this->container->value("alumno")->_fromArray($this->alumnoExistente,"set");
  ...
  public function transferirEntidadCalificacion(){
    ...
    foreach($transferir as $disposicion => $calificacion){
      $tr = $this->container->tools("calificacion")->value($calificacion)["calificacion"];

      if(in_array($disposicion, $existentes)){
        $ex = $this->container->tools("calificacion")->value($existentes[$curso])["calificacion"];
  ...
  public function transferirEntidadDisposicionPendiente_(){
    ...
    foreach($transferir as $disposicion => $disposicionPendiente){
      $tr = $this->container->tools("disposicion_pendiente")->value($disposicionPendiente)["disposicion_pendiente"];

      if(in_array($disposicion, $existentes)){
        $ex = $this->container->tools("disposicion_pendiente")->value($existentes[$curso])["calificacion"];
