...
class GenerarHorarioSqlComision  {
  /**
  ...
  public function main($data) {
    ...
    $h = $this->container->query("horario")->cond(["comision-id","=", $this->id])
    ->size(0)->page(1)->fields(["count"])->columnOne();
    if(intval($h)) throw new Exception("Ya existen horarios para la comision " . $this->id);
  
    $cursos = $this->query("curso")->cond(["comision","=", $idComision])->fields()->all();  
    if(empty($cursos)) throw new Exception("No existen cursos para la comision " . $this->id);
    $curso_asignatura_s = array_combine_key($cursos,"asignatura"); //agrupar cursos por asignatura

    $this->dia_ = $data["dias"];
    if(!shuffle($this->dia_)) throw new Exception("No se pudo asignar un orden aleatorio a los días");
    /**
