...
foreach($comision_cursos_horarios as $comision => $cursos){
  foreach($cursos as $curso => $horarios){
    ...
    foreach($horarios as $horario){
      $h = $container->tools("horario")->value($horario);
...
<? foreach($comision_cursos_horarios as $comision => $cursos):
  ...
  $first2 = true;
  foreach($cursos as $curso => $horarios): 
    $d = $container->tools("horario")->value($horarios[0]);
    $h = $container->tools("horario")->value($horarios[0]) ?>
  <tr>
    ...
    <?if(array_key_exists($curso, $curso_docentes)): $p = $container->tools("toma")->value($curso_docentes[$curso]);?>
