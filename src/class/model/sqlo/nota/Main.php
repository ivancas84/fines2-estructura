...
class NotaSqloMain extends EntitySqlo {
  ...
  public function __construct(){
    ...
    $this->sql = EntitySql::getInstanceFromString('nota');
  }
...
";
  ...
  public function json(array $row){
    ...
    if(!is_null($row['pro_id'])){
      $json = EntitySql::getInstanceFromString('id_persona', 'pro')->_json($row);
      $row_["profesor_"] = $json;
    }
    ...
    if(!is_null($row['alu_id'])){
      $json = EntitySql::getInstanceFromString('id_persona', 'alu')->_json($row);
      $row_["alumno_"] = $json;
    }
    return $row_;
  }
  ...
  public function values(array $row){
    ...
    $row_["nota"] = EntityValues::getInstanceFromString("nota", $json);

    $json = ($row && !is_null($row['pro_id'])) ? EntitySql::getInstanceFromString('id_persona', 'pro')->_json($row) : null;
    ...
    $row_["profesor1"] = EntityValues::getInstanceFromString('id_persona', $json);
    ...
    $json = ($row && !is_null($row['alu_id'])) ? EntitySql::getInstanceFromString('id_persona', 'alu')->_json($row) : null;
    $row_["alumno"] = EntityValues::getInstanceFromString('id_persona', $json);
