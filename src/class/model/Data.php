...
";
  ...
  public static function sqlcomisionesPublicadas($fecha, $orden = "tramo"){
    $render = new Render();
    $advanced = EntitySql::getInstanceFromString("comision")->_advancedComisionesPublicadas($fecha);
    ...
    return EntitySqlo::getInstanceFromString("comision")->all($render);
  }
...
LEFT JOIN (" . EntitySqlo::getInstanceFromString("comision")->cantidadAlumnosActivos($fecha) . ") AS cantidad_alumnos_activos ON (cantidad_alumnos_activos.comision = comisiones_publicadas.id);
";
  }
  ...
  public static function comisionesPublicadasCantidadAlumnos($fecha, $orden = "tramo"){
    ...
    $sql = EntitySqlo::getInstanceFromString("comision")->cantidadAlumnosActivos($fecha);
    $cantidadActivos = Dba::fetchAll($sql);

    $sql = EntitySqlo::getInstanceFromString("comision")->cantidadAlumnosNoActivos($fecha);
  ...
  public static function nominaActivos($fecha){
    ...
    $sql = EntitySqlo::getInstanceFromString("nomina2")->activos($fecha, $render);
  ...
  public static function nominaNoActivos($fecha){
    ...
    $sql = EntitySqlo::getInstanceFromString("nomina2")->noActivos($fecha, $render);
    $rows = Dba::fetchAll($sql);
    return EntitySql::getInstanceFromString("nomina2")->jsonAll($rows);
  }
  ...
  public static function nominaActivosDuplicados($fecha){
    $render = new Render();
    $render->setOrder(["com_dvi_sed_numero"=>"asc", "com_anio" => "asc", "com_semestre" => "asc", "com_dvi_numero" => "asc", "per_apellidos" => "asc", "per_nombres" => "asc"]);
    $sql = EntitySqlo::getInstanceFromString("nomina2")->activosDuplicados($fecha);
    $rows = Dba::fetchAll($sql);
    return EntitySql::getInstanceFromString("nomina2")->jsonAll($rows);
  }

  public static function consolidado($fecha){
    $tomaSql = EntitySql::getInstanceFromString('toma', 'toma');
    $profesorSql = EntitySql::getInstanceFromString('id_persona', 'pro');
    $reemplazoSql = EntitySql::getInstanceFromString('id_persona', 'ree');

    $sqlCursos = EntitySqlo::getInstanceFromString("curso")->all([
    	["com_fecha", "=", $fecha],
    	["com_publicar", "=", true]
    ]);
...
";
    ...
    foreach($rows as $row){
      if($row["id"] != $idCurso) {
        if($row["com_id"] != $idComision) {
          ...
          $i++;
          $j = -1;
          $comisiones[$i] = EntitySql::getInstanceFromString("comision", "com")->_json($row);
          $comisiones[$i]["division_"] =  EntitySql::getInstanceFromString("division", "com_dvi")->_json($row);
          $comisiones[$i]["division_"]["sede_"] =  EntitySql::getInstanceFromString("sede", "com_dvi_sed")->_json($row);
          $comisiones[$i]["division_"]["sede_"]["domicilio_"] =  EntitySql::getInstanceFromString("domicilio", "com_dvi_sed_dom")->_json($row);
          $comisiones[$i]["division_"]["sede_"]["coordinador_"] =  EntitySql::getInstanceFromString("id_persona", "com_dvi_sed_coo")->_json($row);
          $comisiones[$i]["division_"]["plan_"] =  EntitySql::getInstanceFromString("plan", "com_dvi_pla")->_json($row);

          $coordinador = EntitySql::getInstanceFromString("id_persona", "com_dvi_sed_coo")->_json($row);
        ...
        $idCurso = $row["id"];
        $j++;
        $comisiones[$i]["cursos"][$j] = EntitySql::getInstanceFromString("curso")->_json($row);
        $comisiones[$i]["cursos"][$j]["carga_horaria_"] = EntitySql::getInstanceFromString("carga_horaria", "ch")->_json($row);
        $comisiones[$i]["cursos"][$j]["carga_horaria_"]["asignatura_"] = EntitySql::getInstanceFromString("asignatura", "ch_asi")->_json($row);
  ...
  public function buscarAlumnos($search) {
    ...
    public function cantidadAlumnos($fecha){
      $sqlF = EntitySqlo::getInstanceFromString("nomina2")->cantidadActivosPorSexo($fecha, "Femenino");
      $sqlM = EntitySqlo::getInstanceFromString("nomina2")->cantidadActivosPorSexo($fecha, "Masculino");
