...
class ReferenteSqlMain extends EntitySql{
  ...
  public function _fields(){
    ...
    $p = $this->prf();
    return '
' . $this->_mappingFieldEntity($p.'id') . ' AS ' . $p.'id, ' . $this->_mappingFieldEntity($p.'alta') . ' AS ' . $p.'alta, ' . $this->_mappingFieldEntity($p.'baja') . ' AS ' . $p.'baja, ' . $this->_mappingFieldEntity($p.'persona') . ' AS ' . $p.'persona, ' . $this->_mappingFieldEntity($p.'sede') . ' AS ' . $p.'sede';
  }
  ...
  public function _fieldsDb(){
    ...
    $p = $this->prf();
    return '
' . $this->_mappingFieldEntity($p.'id') . ', ' . $this->_mappingFieldEntity($p.'alta') . ', ' . $this->_mappingFieldEntity($p.'baja') . ', ' . $this->_mappingFieldEntity($p.'persona') . ', ' . $this->_mappingFieldEntity($p.'sede') . '';
  }
...
';
  ...
  public function join(Render $render){
    return EntitySql::getInstanceFromString('id_persona', 'per')->_join('persona', 'refe', $render) . '
' . EntitySql::getInstanceFromString('sede', 'sed')->_join('sede', 'refe', $render) . '
...
' ;
  ...
  public function initializeInsert(array $data){
    $data['id'] = (!empty($data['id'])) ? $data['id'] : Dba::nextId('referente');
